...
<#if reportIsOpened>
    <#if currentUser.role == "ROLE_ADMIN">
        <div><a href="/reports/month/${year?c}-${monthNum?string["00"]}/options" class="btn btn-success">Закрыть месяц</a></div>
    </#if>
            ...
            <tr>
                ...
                <#list orders as order>
                    <#list order.consumes as consume>
                        <#assign sumPriceConsumes += consume.product.price * consume.actualUsedQuantity>
                    </#list>
                </#list>
            ...
            <tr>
                ...
                <#list users?keys as userId>
                    <#if users[userId].role == 'ROLE_PAINTER'>
                        <#assign sumBonus += options['BONUS_PAINTER']?eval * areaByUserId[userId]>
                    </#if>
                    <#if users[userId].role == 'ROLE_LABORER'>
                        <#assign sumBonus += options['BONUS_LABORER']?eval * areaByUserId[userId]>
                    </#if>
                </#list>
    ...
    <#list teams?keys as teamId>
        <tr>
            ...
            <td>
                ...
                <#list orders as order>
                    <#list order.consumes as consume>
                        <#assign sumConsumesQuantity += consume.actualUsedQuantity>
                    </#list>
                </#list>
...
<#else>
    <#include "month-report-closed.ftl">
</#if>
<#include "../footer.ftl">